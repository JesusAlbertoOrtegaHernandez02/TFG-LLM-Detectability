...
# Notebook de generación y selección de textos para una iteración concreta.
# Carga un adaptador LoRA ya entrenado, genera abstracts y selecciona los textos
# más cercanos al comportamiento estadístico humano.

# Instalación para las dependencias necesarias en Google Colab
!pip install -q transformers peft pandas matplotlib tqdm torch

# Por si sucediara algun fallo referente a bitsandbytes se actualiza
...
# Esta sección se encarga del cargado del modelo, el valor de N en la variable ITERATION
# sirve para indicar en todo el proceso en que iteración se está trabajando.
# Si por ejemplo se quiere generar iter1 --> ITERATION = 1

ITERATION = N

model_id = "mistralai/Mistral-7B-v0.1"

# Cuantitación en 4 bits para cargar el modelo con menor consumo de memoria
...
# Cargar el adaptador LoRA en la iteración seleccionada
...
def is_complete(text: str) -> bool:
  """
  Se centra en comprobar que el texto generado parezca completo:
  Si termina en signo de cierre. contiene varias frases y alcanza la longitud máxima es valido.
  """
    text = text.strip()
    ...
    with torch.no_grad():
      output_ids = model.generate(
        **inputs,
        ...
        # Ajuste de parámetros  para equilibrar la coherencia y diversidad
        temperature=1.02,
        top_p=0.93,
        top_k=70,

        # Penalización para reducir repeticiones sin eliminar la variabilidad
        repetition_penalty=1.05,  #  bajar para tener más naturalidad
...
def compute_perplexity(text: str) -> float:
  ...
  """
  Calcula la perplexity de los textos generados usando GPT-2 como modelo de referencia
  """
...
def gpt_detector_score(text: str) -> float
    """
    Devuelve la probabilidad asignada por el modelo RoBERTa
    """
...
# Modelo de referencia para calcular Perplexity y ranking GLTR.
gltr_tok = AutoTokenizer.from_pretrained("gpt2")
gltr_model = AutoModelForCausalLM.from_pretrained("gpt2").to("cuda")
gltr_model.eval()

# Detector externo basado en RoBERTa usado como métrica complementaria.
...
def gltr_scores(text: str):
    """
    Calcula métricas tipo GLTR a partir de la probabilidad de cada token
    """
...
# Se carga el conjunto base humano para que sus métricas se utilicen como centro estadístico
...
# Se calculan las métricas sobre los textos humanos para obtener media y desviación estándar
...
# Se generan los prompts estáticos e imparciales para las demás iteraciones
abstract_prompts = [
    "Abstract:",
...
N_GENERATIONS = 35 # Número de generaciones por cada prompt en la ejecución
...
# Generación de textos para la iteración seleccionada
rows = []
id_counter = 0
...
csv_metrics = f"{METRICS_PATH}/generated_iter{ITERATION}_metrics.csv"
df.to_csv(csv_metrics, index=False)
print("Métricas completas guardadas en:", csv_metrics)
...
filtered = filtered[
    filtered["perplexity"].between(ppl_low, ppl_high)
]

filtered = filtered[
    filtered["perplexity"].between(ppl_low, ppl_high) &
    filtered["gltr_mean_rank"].between(gltr_low, gltr_high)
]

if len(filtered) == 0:
    raise ValueError("No quedan textos tras los filtros. Relaja perplexity, GLTR o filtros básicos.")
...
filtered["human_distance"] = np.sqrt(
    filtered["z_ppl"]**2 + 1.2 * filtered["z_gltr"]**2
)
...
# 70% mejores por cercanía humana
top_k = min(int(N_SAMPLES * 0.7), len(filtered))


top_selected = filtered.sort_values(
    "human_distance",
    ascending=True
).head(top_k)

# 30% textos variados, pero todavía ponderados por cercanía
remaining = filtered.drop(top_selected.index)

diverse_n = N_SAMPLES - len(top_selected)

if len(remaining) > 0 and diverse_n > 0:
    diverse_selected = remaining.sample(
        n=min(diverse_n, len(remaining)),
        weights="selection_weight",
        random_state=42
    )

    selected = pd.concat([top_selected, diverse_selected])
else:
    selected = top_selected.copy()

selected = selected.sample(frac=1, random_state=42).reset_index(drop=True)
selected = selected.sort_values("human_distance")

print("\n=== COMPARACIÓN FILTRADOS VS SELECCIONADOS ===")
print("Filtered PPL:", filtered["perplexity"].mean(), "±", filtered["perplexity"].std())
print("Selected PPL:", selected["perplexity"].mean(), "±", selected["perplexity"].std())

print("Filtered GLTR:", filtered["gltr_mean_rank"].mean(), "±", filtered["gltr_mean_rank"].std())
print("Selected GLTR:", selected["gltr_mean_rank"].mean(), "±", selected["gltr_mean_rank"].std())

print("Filtered distance:", filtered["human_distance"].mean(), "±", filtered["human_distance"].std())
print("Selected distance:", selected["human_distance"].mean(), "±", selected["human_distance"].std())
...
# Visualización y resumen final de las métricas obtenidas.
# Estas gráficas permiten comparar la distribución completa frente a los textos seleccionados.
